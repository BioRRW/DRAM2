...
@click.group(
    "adjectives",
    cls=OrderedGroup,
    help="Describe Gene Features (Adjectives)\n\n The DRAM2 Adjectives are features, defined by a set of rules which them selves are based on key genes and selective use of phylogenetic trees.",
)
@click.pass_context
def adjectives_cmd(
    ctx: click.Context,
...
@click.command("eval")
@click.argument(
    "annotations_tsv", type=click.Path(exists=True)
)  # , required=1, help="One of only 2 required files. Path to a DRAM annotations file.")
@click.argument(
    "adjectives_tsv", type=click.Path(), required=1
)  # , help="One of only 2 required files.Path for the output true false table created by this script.")
@click.option(
    "-a",
    "--adjectives",
    multiple=True,
...
def evaluate(
    ...
    plot_adjectives: list = None,
    plot_genomes: list = None,
    plot_path: str = None,
    debug_ids_by_fasta_to_tsv: str = None,
    strainer_tsv: str = None,
    strainer_type: str = "pgtb",
):
    """
...
)  # , help='The rules file which adhere to strict formating' )
@click.option(
    "--list_name",
    is_flag=True,
    callback=list_adjective_name,
...
def plot_rules_cmd(
    ...
    Understanding how annotations uses it rules to call a given gene is non-trivial. This function provides a method to name these genes. 
