...
from dram2.db_kits.kegg_kit import KeggKit
import pandas as pd
...
def test_check_setup_fail(dram_context_bad, db_args_dict):
    with pytest.raises(
        ...
        kit = KeggKit(dram_context_bad.get_dram_config(), db_args_dict)



def test_check_setup_pass(dram_context, db_args_dict):
    kit = KeggKit(dram_context.get_dram_config(), db_args_dict)
...
def test_search_cmd(tmpdir, runner):
    ...
    result1 = runner.invoke(
        dram2, ["-o", output_dir, "annotate", "--use_db", "kegg", input_faa]
    )
    result = runner.invoke(
        dram2, ["-o", output_dir_empty, "annotate", "--use_db", "kegg", input_faa]
    )
    # breakpoint()
    # assert pd.read_csv(
