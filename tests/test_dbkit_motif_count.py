...
from dram2.db_kits.motif_count import CountMotifsKit
import pandas as pd
...
@pytest.fixture()
def tmp_fasta(logger, tmp_path) -> Fasta:
...
def test_check_setup_fail(dram_context, db_args_dict):
    with pytest.raises(
        FileNotFoundError,
        match=(
        ...
        kit = CountMotifsKit(dram_context.get_dram_config(), db_args_dict)


def test_check_setup_pass(context, db_args_dict):
    kit = CountMotifsKit(context.get_dram_config(), db_args_dict)


def test_search(context, tmp_fasta, db_args_dict):

    kit = CountMotifsKit(context.get_dram_config(), db_args_dict)
    out_received = kit.search(tmp_fasta)
    output_dir = context.get_output_dir()
    # assert pd.read_csv(
    #     Path(output_dir, "annotations.tsv"), sep="\t", index_col=0
    # ).equals(
    #     pd.read_csv(
    #         Path("tests", "data", "test_annotations.tsv"),
    #         sep="\t",
    #         index_col=0,
    #     )
    # )
...
def test_search_cmd(tmpdir, runner):
    input_faa = Path("tests", "data", "test_genes.faa")
    input_faa_empty = Path("tests", "data", "test_genes_empty.faa")
    output_dir = Path(tmpdir, "annotations")
    output_dir_empty = Path(tmpdir, "annotations_empty")
    result1 = runner.invoke(
        dram2, ["-o", output_dir, "annotate", "--use_db", "motif", input_faa]
    )
    # assert result1.exit_code == 0
    result = runner.invoke(
        dram2, ["-o", output_dir_empty, "annotate", "--use_db", "motif", input_faa]
    )
    # breakpoint()
    # assert pd.read_csv(
