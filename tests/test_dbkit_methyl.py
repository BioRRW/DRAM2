...
from dram2.db_kits.methyl_kit import MethylKit
import pandas as pd
...
def context(tmpdir):
    dram_context = DramContext(
        cores=10,
        db_path=tmpdir,
        config_file=Path("tests/data/methyl_config.yaml"),
        log_file_path=tmpdir / "log",
        output_dir=tmpdir / "out1",
        keep_tmp=False,
        verbose=0,
...
def test_check_setup_fail(dram_context_bad, db_args_dict):
    with pytest.raises(
        ...
        kit = MethylKit(dram_context_bad.get_dram_config(), db_args_dict)



def test_check_setup_pass(dram_context, db_args_dict):
    kit = MethylKit(dram_context.get_dram_config(), db_args_dict)
...
def test_search_cmd(tmpdir, runner):
    ...
    result1 = runner.invoke(
        dram2, ["-o", output_dir, "annotate", "--use_db", "methyl", input_faa]
    )
    # assert result1.exit_code == 0
    result = runner.invoke(
        dram2, ["-o", output_dir_empty, "annotate", "--use_db", "methyl", input_faa]
    )
    # breakpoint()
    # assert pd.read_csv(
